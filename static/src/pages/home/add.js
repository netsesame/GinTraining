loader.define(function(require, exports, module) {
    ...
    pageview.bindExchange = function(val) {
        ...
        if (tmpArray) {
            ...
            bui.ajax({
                ...
                var tmpArray = $.map(result.data, function(value, key) {
                    return key + ' (' + value + ')';
                });
    ...
    bui.btn("#submit").submit(function(loading) {
        ...
        if (bs.$data.m.exchange == '') {
            bui.alert('请选择市场！');
            loading.stop();
            return;
        }
        if (bs.$data.m.api_key == '') {
            bui.alert('请填写apikey');
            loading.stop();
            return;
        }
        ...
        if (bs.$data.m.order_type == '') {
            bui.alert('请设置订单类型');
            loading.stop();
            return;
        }
        if (bs.$data.m.buy_float == '') {
            bui.alert('请设置买入浮动');
            loading.stop();
            return;
        }
        if (bs.$data.m.sell_float == '') {
            bui.alert('请设置卖出浮动');
            loading.stop();
            return;
        }
        if (bs.$data.m.nickname == '') {
            bui.alert('请输入备注名称');
            loading.stop();
            return;
        }
